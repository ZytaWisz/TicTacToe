    ...
    public String checkGameWinner(char [][]grid){
        ...
        for( int i=0; i<=2; i++){
            if (grid[0][i]==grid[1][i] && grid[1][i]==grid[2][i] && (grid[0][i]=='x' || grid[0][i]=='o')){
                return String.valueOf(grid[0][i]).toUpperCase()+" wins";
            }
        }
        for( int i=0; i<=2; i++){
            if (grid[i][0]==grid[i][1] && grid[i][1]==grid[i][2] && (grid[i][0]=='x' || grid[i][0]=='o')){
                return String.valueOf(grid[i][0]).toUpperCase()+" wins";
            }
        }
        if (grid[0][0]==grid[1][1] && grid[1][1]==grid[2][2] && (grid[0][0]=='x' || grid[0][0]=='o')){
            return String.valueOf(grid[0][0]).toUpperCase()+" wins";
        }
        if (grid[0][2]==grid[1][1] && grid[1][1]==grid[2][0] && (grid[0][2]=='x' || grid[0][2]=='o')){
            return String.valueOf(grid[0][2]).toUpperCase()+" wins";
        }
